...
void Demo::build_secondary_cmd()
{
	...
	// Draw 2D first, so that the depth buffer ignores
	// pixels behind the 2D overlay, and also because
	// no vertex or index buffer has been bound yet

	// 2D Pipeline
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipe2D->pipeline);

	// 2D Logo
	logoEntity->Draw(cmd, pipe2D->pipeline_layout);
	textEntity->Draw(cmd, pipe2D->pipeline_layout);
	textEntity2->Draw(cmd, pipe2D->pipeline_layout);
...
void Demo::prepare()
{
	...
	if (firstInit)
	{
		...
		logoTex = new Texture("../../../Assets/Logo.png", this);
		fontTex = new Texture("../../../Assets/font2.png", this);
		...
		logoEntity = new Entity(this);
		logoEntity->texture[0] = logoTex;
		logoEntity->CreateDescriptorSet2D(this);

		textEntity = new Entity(this);
		textEntity->texture[0] = fontTex;
		sprintf(textEntity->name, "_______");
		textEntity->CreateDescriptorSet2D(this);

		textEntity2 = new Entity(this);
		textEntity2->texture[0] = fontTex;
		sprintf(textEntity2->name, "_______");
		textEntity2->CreateDescriptorSet2D(this);
	}
...
void Demo::draw()
{
	...
	catEntity->Update3D(projection_matrix, view_matrix);
	dogEntity->Update3D(projection_matrix, view_matrix);
	buildingEntity->Update3D(projection_matrix, view_matrix);
	skyEntity->Update3D(projection_matrix, view_matrix);

	// width and height here, are window dimensions
	logoEntity->Update2D(width, height);

	textEntity->Update2D(width, height);
	textEntity2->Update2D(width, height);
...
Demo::~Demo()
{
	...
	delete logoTex->textureGPU;
	delete fontTex->textureGPU;
